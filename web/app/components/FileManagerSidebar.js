  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 scrollbar-thin">
        ...
        {/* Bin Tab */}
        <div
          className={`group flex items-center gap-2 px-3 py-2 rounded-xl transition-colors mt-1 ${
            currentPath === ".bin" ? "bg-purple-100 text-purple-700" : "text-slate-700 hover:bg-slate-100"
          }`}
        >
          <button
            onClick={() => onNavigate(".bin")}
            className={`flex-1 flex items-center gap-3 min-w-0 text-left text-sm ${currentPath === ".bin" ? "font-medium" : ""}`}
          >
