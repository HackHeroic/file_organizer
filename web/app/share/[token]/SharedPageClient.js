...
export default function SharedPageClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const shareRes = await fetch(`${API_BASE}/api/file-manager/share?token=${encodeURIComponent(token)}`);
        ...
        if (file) {
          ...
          const listRes = await fetch(`${API_BASE}/api/file-manager/list?path=${encodeURIComponent(shareData.path)}`);
              ...
              items.map((item) => {
                ...
                const handleClick = () => {
                  ...
                  if (canView) setViewFile({ path: item.path, name: item.name });
                    ...
                    a.href = `${API_BASE}/api/file-manager/download?path=${encodeURIComponent(item.path)}`;
